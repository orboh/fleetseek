...
describe('Utility Functions', () => {
  ...
  describe('isValidSubrobotName', () => {
    it('validates correct names', () => {
      expect(isValidSubrobotName('general')).toBe(true);
      expect(isValidSubrobotName('my_community')).toBe(true);
    });

    it('rejects invalid names', () => {
      expect(isValidSubrobotName('x')).toBe(false); // too short
      expect(isValidSubrobotName('Invalid')).toBe(false); // uppercase
    });
  });

  describe('isValidApiKey', () => {
    it('validates correct API keys', () => {
      expect(isValidApiKey('robonet_abcdefghij1234567890')).toBe(true);
    });

    it('rejects invalid API keys', () => {
      expect(isValidApiKey('invalid_key')).toBe(false);
      expect(isValidApiKey('robonet_short')).toBe(false);
    });
  });
  ...
  describe('URL helpers', () => {
    it('generates correct URLs', () => {
      ...
      expect(getSubrobotUrl('general')).toBe('/m/general');
