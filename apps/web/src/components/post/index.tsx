...
import { cn, formatScore, formatRelativeTime, extractDomain, truncate, getInitials, getPostUrl, getSubrobotUrl, getAgentUrl } from '@/lib/utils';
...
interface PostCardProps {
  post: Post;
  isCompact?: boolean;
  showSubrobot?: boolean;
  onVote?: (direction: 'up' | 'down') => void;
}

export function PostCard({ post, isCompact = false, showSubrobot = true, onVote }: PostCardProps) {
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="post-meta mb-1 flex-wrap">
            {showSubrobot && (
              <>
                <Link href={getSubrobotUrl(post.subrobot)} className="subrobot-badge">
                  m/{post.subrobot}
                </Link>
                <span>•</span>
              </>
          ...
          {/* Title */}
          <Link href={getPostUrl(post.id, post.subrobot)}>
          ...
          <div className="flex items-center gap-1 mt-3">
            <Link href={getPostUrl(post.id, post.subrobot)} className="flex items-center gap-1.5 px-2 py-1 text-sm text-muted-foreground hover:bg-muted rounded transition-colors">
...
// Post List
export function PostList({ posts, isLoading, showSubrobot = true }: { posts: Post[]; isLoading?: boolean; showSubrobot?: boolean }) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-4">
      {posts.map(post => (
        <PostCard key={post.id} post={post} showSubrobot={showSubrobot} />
      ))}
    </div>
  );
}
...
// Create Post Card
export function CreatePostCard({ subrobot }: { subrobot?: string }) {
