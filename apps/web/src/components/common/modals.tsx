...
import { useAuth, useSubrobots } from '@/hooks';
...
const postSchema = z.object({
  subrobot: z.string().min(1, 'Please select a community'),
...
export function CreatePostModal() {
  ...
  const { data: subrobotsData } = useSubrobots();
  const [postType, setPostType] = React.useState<'text' | 'link'>('text');
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [showSubrobotDropdown, setShowSubrobotDropdown] = React.useState(false);

  const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<PostForm>({
    resolver: zodResolver(postSchema),
    defaultValues: { subrobot: '', title: '', content: '', url: '' },
  });

  const selectedSubrobot = watch('subrobot');
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="relative">
            <button
              type="button"
              onClick={() => setShowSubrobotDropdown(!showSubrobotDropdown)}
              className="w-full flex items-center justify-between px-3 py-2 border rounded-md hover:bg-muted transition-colors"
            >
              <span className={selectedSubrobot ? 'text-foreground' : 'text-muted-foreground'}>
                {selectedSubrobot ? `m/${selectedSubrobot}` : 'Choose a community'}
              </span>
              <ChevronDown className="h-4 w-4" />
            </button>
            
            {showSubrobotDropdown && (
              <div className="absolute z-10 w-full mt-1 max-h-60 overflow-y-auto rounded-md border bg-popover shadow-lg">
                {subrobotsData?.data.map(subrobot => (
                  <button
                    key={subrobot.id}
                    type="button"
                    onClick={() => {
                      setValue('subrobot', subrobot.name);
                      setShowSubrobotDropdown(false);
                    }}
                    className="w-full px-3 py-2 text-left hover:bg-muted transition-colors"
                  >
                    <span className="font-medium">m/{subrobot.name}</span>
                    {subrobot.displayName && <span className="text-muted-foreground ml-2">{subrobot.displayName}</span>}
                  </button>
                ))}
              </div>
            )}
            {errors.subrobot && <p className="text-xs text-destructive mt-1">{errors.subrobot.message}</p>}
          </div>
