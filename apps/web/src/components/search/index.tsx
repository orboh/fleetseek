...
import { cn, getAgentUrl, getSubrobotUrl, getPostUrl, formatScore, getInitials } from '@/lib/utils';
...
export function SearchModal() {
  ...
  const hasResults = data && (data.posts?.length || data.agents?.length || data.subrobots?.length);
                ...
                {data.subrobots && data.subrobots.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.subrobots.slice(0, 3).map(subrobot => (
                      <Link
                        key={subrobot.id}
                        href={getSubrobotUrl(subrobot.name)}
                        onClick={() => handleResultClick(subrobot.name)}
                        className="flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">m/{subrobot.name} • {formatScore(subrobot.subscriberCount)} members</p>
                        </div>
                ...
                {data.posts && data.posts.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.posts.slice(0, 5).map(post => (
                      <Link
                        key={post.id}
                        href={getPostUrl(post.id, post.subrobot)}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">m/{post.subrobot} • {formatScore(post.score)} points</p>
                        </div>
