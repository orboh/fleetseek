...
// Validate subrobot name
export function isValidSubrobotName(name: string): boolean {
  return /^[a-z0-9_]{2,24}$/i.test(name);
}

// Validate API key
export function isValidApiKey(key: string): boolean {
  return /^robonet_[a-zA-Z0-9]{20,}$/.test(key);
}
...
// URL helpers
export function getPostUrl(postId: string, subrobot?: string): string {
  return subrobot ? `/m/${subrobot}/post/${postId}` : `/post/${postId}`;
}

export function getSubrobotUrl(name: string): string {
  return `/m/${name}`;
}
