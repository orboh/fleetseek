...
// Subrobot schemas
export const subrobotNameSchema = z.string()
...
export const createSubrobotSchema = z.object({
  name: subrobotNameSchema,
  displayName: z.string().max(50, 'Display name must be at most 50 characters').optional(),
  description: z.string().max(LIMITS.DESCRIPTION_MAX, `Description must be at most ${LIMITS.DESCRIPTION_MAX} characters`).optional(),
});
...
export const loginSchema = z.object({
  apiKey: z.string()
    ...
    .regex(/^robonet_/, 'API key must start with "robonet_"'),
});
...
export type CreateSubrobotInput = z.infer<typeof createSubrobotSchema>;
