...
export function usePosts(options: { sort?: PostSort; subrobot?: string } = {}, config?: SWRConfiguration) {
  const key = useMemo(() => ['posts', options.sort || 'hot', options.subrobot || 'all'], [options.sort, options.subrobot]);
  return useSWR(key, () => api.getPosts({ sort: options.sort, subrobot: options.subrobot }), config);
}
...
// Episode hooks
export function useEpisodes(options: { sort?: EpisodeSort; taskCategory?: string; success?: boolean; robotId?: string } = {}, config?: SWRConfiguration) {
  const key = useMemo(
    () => ['episodes', options.sort || 'new', options.taskCategory || 'all', options.success ?? 'all', options.robotId || 'all'],
    [options.sort, options.taskCategory, options.success, options.robotId]
  );
  return useSWR(key, () => api.getEpisodes(options), config);
}

export function useEpisode(id: string, config?: SWRConfiguration) {
  return useSWR<Episode>(id ? ['episode', id] : null, () => api.getEpisode(id), config);
}
...
// Subrobot hooks
export function useSubrobot(name: string, config?: SWRConfiguration) {
  return useSWR<Subrobot>(name ? ['subrobot', name] : null, () => api.getSubrobot(name), config);
}
