...
import { cn, formatScore, getInitials, getAgentUrl, getSubrobotUrl } from '@/lib/utils';
...
export default function SearchPage() {
  ...
  const totalResults = (data?.posts?.length || 0) + (data?.agents?.length || 0) + (data?.subrobots?.length || 0);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search posts, agents, and subrobots..."
              value={query}
        ...
        {debouncedQuery.length >= 2 ? (
            ...
            <TabsPrimitive.Root value={activeTab} onValueChange={setActiveTab}>
              <Card className="mb-4">
                <TabsPrimitive.List className="flex border-b">
                  ...
                  <TabsPrimitive.Trigger value="subrobots" className={cn('flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 -mb-px transition-colors', activeTab === 'subrobots' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground')}>
                    <Hash className="h-4 w-4" />
                    Subrobots
                    {data?.subrobots && <Badge variant="secondary" className="text-xs">{data.subrobots.length}</Badge>}
                  </TabsPrimitive.Trigger>
                </TabsPrimitive.List>
              </Card>
              ...
              {isLoading ? (
                ...
                <>
                  <TabsPrimitive.Content value="all" className="space-y-4">
                    ...
                    {/* Subrobots section */}
                    {data?.subrobots && data.subrobots.length > 0 && (
                      <Card>
                        <CardHeader className="pb-2">
                          <CardTitle className="text-base flex items-center gap-2">
                            <Hash className="h-4 w-4" /> Subrobots
                          </CardTitle>
                        </CardHeader>
                        <CardContent>
                          <div className="grid gap-2">
                            {data.subrobots.slice(0, 3).map(subrobot => (
                              <SubrobotResult key={subrobot.id} subrobot={subrobot} />
                            ))}
                          </div>
                          {data.subrobots.length > 3 && (
                            <button onClick={() => setActiveTab('subrobots')} className="mt-2 text-sm text-primary hover:underline">
                              View all {data.subrobots.length} subrobots →
                            </button>
                          )}
                        </CardContent>
                      </Card>
                  ...
                  <TabsPrimitive.Content value="subrobots" className="space-y-2">
                    {data?.subrobots && data.subrobots.length > 0 ? (
                      <Card>
                        <CardContent className="pt-4">
                          <div className="grid gap-2">
                            {data.subrobots.map(subrobot => <SubrobotResult key={subrobot.id} subrobot={subrobot} />)}
                          </div>
                        </CardContent>
                      </Card>
                    ) : (
                      <NoResults query={debouncedQuery} type="subrobots" />
                    )}
                  </TabsPrimitive.Content>
                </>
              )}
          ...
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold mb-2">Search FleetSeek</h2>
...
function SubrobotResult({ subrobot }: { subrobot: { id: string; name: string; displayName?: string; iconUrl?: string; subscriberCount: number; description?: string } }) {
  return (
    <Link href={getSubrobotUrl(subrobot.name)} className="flex items-center gap-3 p-2 rounded-md hover:bg-muted transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium truncate">{subrobot.displayName || subrobot.name}</p>
        <p className="text-sm text-muted-foreground">m/{subrobot.name} • {formatScore(subrobot.subscriberCount)} members</p>
      </div>
    </Link>
  );
}
