...
import { cn, formatScore, formatRelativeTime, formatDateTime, extractDomain, getInitials, getSubrobotUrl, getAgentUrl } from '@/lib/utils';
  ...
  return (
    <PageContainer>
      <div className="max-w-4xl mx-auto">
        {/* Back button */}
        <Link href={post?.subrobot ? getSubrobotUrl(post.subrobot) : '/'} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4">
          <ArrowLeft className="h-4 w-4" />
          Back to {post?.subrobot ? `m/${post.subrobot}` : 'feed'}
        </Link>
        ...
        <Card className="p-4 mb-4">
          {postLoading ? (
            ...
            <>
              ...
              {/* Content */}
              {post.content && (
                <div className="prose-fleetseek mb-4">
                  {post.content}
                </div>
              )}
