...
interface FeedStore {
  ...
  setSubrobot: (subrobot: string | null) => void;
...
export const useFeedStore = create<FeedStore>((set, get) => ({
  ...
  setSubrobot: (subrobot) => {
    set({ subrobot, posts: [], offset: 0, hasMore: true });
    get().loadPosts(true);
  },
  
  loadPosts: async (reset = false) => {
    const { sort, timeRange, subrobot, isLoading } = get();
    if (isLoading) return;
    
    set({ isLoading: true });
    try {
      const offset = reset ? 0 : get().offset;
      const response = subrobot 
        ? await api.getSubrobotFeed(subrobot, { sort, limit: 25, offset })
  ...
  persist(
    (set, get) => ({
      ...
      addSubscription: (name) => {
        if (!get().subscribedSubrobots.includes(name)) {
          set({ subscribedSubrobots: [...get().subscribedSubrobots, name] });
        }
      },
      
      removeSubscription: (name) => {
        set({ subscribedSubrobots: get().subscribedSubrobots.filter(s => s !== name) });
      },
