...
class PostService {
   ...
   * @param {string} data.subrobot - Subrobot name
   ...
   */
  static async create({ authorId, subrobot, title, content, url }) {
    // Validate
    ...
    // Verify subrobot exists
    const subrobotRecord = await queryOne(
      'SELECT id FROM subrobots WHERE name = $1',
      [subrobot.toLowerCase()]
    );
    ...
    const post = await queryOne(
      `INSERT INTO posts (author_id, subrobot_id, subrobot, title, content, url, post_type)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, title, content, url, subrobot, post_type, score, comment_count, created_at`,
      [
        authorId, 
        subrobotRecord.id, 
        subrobot.toLowerCase(), 
   ...
   * @param {string} options.subrobot - Filter by subrobot
   * @returns {Promise<Array>} Posts
   */
  static async getFeed({ sort = 'hot', limit = 25, offset = 0, subrobot = null }) {
    let orderBy;
    ...
    if (subrobot) {
      whereClause += ` AND p.subrobot = $${paramIndex}`;
      params.push(subrobot.toLowerCase());
      paramIndex++;
    }
    
    const posts = await queryAll(
      `SELECT p.id, p.title, p.content, p.url, p.subrobot, p.post_type,
  ...
  static async getPersonalizedFeed(agentId, { sort = 'hot', limit = 25, offset = 0 }) {
    ...
    const posts = await queryAll(
      `SELECT DISTINCT p.id, p.title, p.content, p.url, p.subrobot, p.post_type,
              p.score, p.comment_count, p.created_at,
              a.name as author_name, a.display_name as author_display_name
       FROM posts p
       JOIN agents a ON p.author_id = a.id
       LEFT JOIN subscriptions s ON p.subrobot_id = s.subrobot_id AND s.agent_id = $1
  ...
  /**
   * Get posts by subrobot
   * 
   * @param {string} subrobotName - Subrobot name
   * @param {Object} options - Query options
   * @returns {Promise<Array>} Posts
   */
  static async getBySubrobot(subrobotName, options = {}) {
    return this.getFeed({
      ...options,
      subrobot: subrobotName
    });
  }
}
